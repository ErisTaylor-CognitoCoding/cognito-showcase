# Automated Clip Saver for Creatures of Sonaria

> 📺 **[Watch on YouTube →](https://youtu.be/dM20TDLq0sU?si=aN2G6GXCZ78ykGz0)**

> 🤖 **Built with Replit** — Python app built by directing an AI. [Try Replit →](https://replit.com/refer/cognitocoding)

**Live Replit Project:** *(URL not captured — check the video description, or ask in the comments if you'd like the link)*

---

## The Prompt I Gave

*(Full original prompt not captured for this episode — see the video for the brief.)*

> Built a Replit app that watches Creatures of Sonaria gameplay and auto-saves the best clips. Drag-drop the kill-confirms straight to a sleek mini-app — no more scrubbing through footage looking for the good bits.

---

## What the AI Built

A Python app that monitors Creatures of Sonaria gameplay and auto-saves the best moments. When a kill-confirm trigger fires, the app grabs the clip with surrounding context and surfaces it in a sleek review interface. Drag to keep, swipe to discard. Everything runs inside Replit — no desktop software needed.

---

## Series

**Tue Vibe Coding** — one game or app per episode, built live by directing an AI. Every Tuesday on [Cognito Coding](https://www.youtube.com/@CognitoCoding01).

| Ep | Title | Watch |
|---|---|---|
| 01 | Automated Clip Saver for Creatures of Sonaria | [▶ Watch](https://youtu.be/dM20TDLq0sU?si=aN2G6GXCZ78ykGz0) |

---

← [Back to Vibe series index](../README.md)
